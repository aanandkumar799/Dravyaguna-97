...
PUBLIC_PAGES=[
    ...
    "viva.html","progress.html","rasa.html","reference-library.html",
    "practical-lab.html","references.html","404.html"
]
...
for path in sorted(DB.glob("*.json")):
    ...
    if not isinstance(data,dict):
        raise SystemExit(f"{path}: record must be a JSON object")
    records.append(data)
...
if not INDEX.exists():
    errors.append("Missing plant-index.json")
else:
