...
def merge(dst, src):
    for key, value in src.items():
        if isinstance(value, dict):
            ...
            # This batch is an explicit user-supplied enrichment pass, so replace
            # draft values for the supplied fields. Classical text is only included
            # where independently checked; uncertain verses remain empty.
            dst[key] = value
...
for plant in plants:
    ...
    if name in BATCH:
        ...
        plant.setdefault('metadata', {})['batch_49_53_added'] = True
        found.add(name)
...
print('Applied batch 49–53 data to:', ', '.join(sorted(found)))
