...
errors, warnings = [], []
if len(records) != 97:
    errors.append(f'data/plants must contain exactly 97 JSON plant records; found {len(records)}')
if len(core) != 97:
...
for label, vals in [('name', names), ('id', ids), ('order', orders)]:
    ...
    if dupes: errors.append(f'Duplicate {label} values: {dupes}')
...
for p in core:
    ...
    for field, value in required.items():
        if not isinstance(value, str) or not value.strip(): errors.append(f'{name}: missing {field}')
    for field, value in [('rasa', p.get('dravya_guna', {}).get('rasa')), ('guna', p.get('dravya_guna', {}).get('guna')), ('useful_part', p.get('therapeutics', {}).get('useful_part'))]:
        if not isinstance(value, list) or not value: errors.append(f'{name}: {field} must be a non-empty list')
    cr = p.get('classical_reference', {}) or {}
    shlokas, refs = cr.get('shlokas') or [], cr.get('nighantu_references') or []
    if shlokas and not refs: warnings.append(f'{name}: shloka text exists but reference metadata is missing')
    if shlokas and any(not isinstance(s, (str, dict)) for s in shlokas): errors.append(f'{name}: invalid classical shloka item')
    for source in p.get('sources', []) or []:
        if not isinstance(source, dict): errors.append(f'{name}: source entry is not an object')

print(f'QUALITY AUDIT: {len(core)} NCISM records in {len(files)} plant files, {len(errors)} blocking errors, {len(warnings)} warnings')
if warnings:
    print('WARNINGS:')
    for item in warnings[:250]: print(f' - {item}')
if errors:
    print('ERRORS:')
