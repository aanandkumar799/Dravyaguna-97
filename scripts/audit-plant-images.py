...
def commons_candidates(botanical,part):
    ...
    url='https://commons.wikimedia.org/w/api.php?'+urllib.parse.urlencode({'action':'query','generator':'search','gsrsearch':query,'gsrnamespace':6,'gsrlimit':30,'prop':'imageinfo','iiprop':'url|extmetadata|mime','iiurlwidth':1200,'format':'json','origin':'*'})
    data=fetch_json(url);out=[]
    for p in (data.get('query',{}).get('pages',{}) or {}).values():
        info=(p.get('imageinfo') or [{}])[0];title=str(p.get('title') or '');mime=str(info.get('mime') or '')
        lic=str((info.get('extmetadata') or {}).get('LicenseShortName',{}).get('value') or '').strip();score=candidate_score(title,botanical,part)
        if score<0 or not mime.startswith('image/') or not LICENSE_OK.search(re.sub(r'\s+',' ',lic)):continue
        u=info.get('thumburl') or info.get('url')
        if not u:continue
        out.append({'score':score,'url':u,'page':'https://commons.wikimedia.org/wiki/'+urllib.parse.quote(title.replace(' ','_')),'title':title.replace('File:','',1),'license':lic,'author':str((info.get('extmetadata') or {}).get('Artist',{}).get('value') or '').strip(),'source':'Wikimedia Commons'})
...
def inaturalist_candidate(botanical,part):
    ...
    for obs in data.get('results',[]):
        ...
        for ph in obs.get('photos',[]):
            ...
            if u.startswith('http'):return {'score':90,'url':u,'page':'https://www.inaturalist.org/observations/'+str(obs.get('id')),'title':'iNaturalist observation','license':lic.upper(),'author':str(ph.get('attribution') or ''),'source':'iNaturalist'}
    return None
...
def gbif_candidate(botanical,part):
    ...
    for rec in data.get('results',[]):
        ...
        for media in rec.get('media',[]) or []:
            ...
            if u.startswith('http'):return {'score':80,'url':u,'page':'https://www.gbif.org/occurrence/'+str(rec.get('key')),'title':'GBIF occurrence media','license':rec.get('license') or 'See source','author':media.get('creator') or rec.get('recordedBy') or '','source':'GBIF'}
    return None

def main():
    idx=json.loads(INDEX.read_text(encoding='utf-8'));plants=json.loads(PLANTS.read_text(encoding='utf-8'))
    manifest=json.loads(MANIFEST.read_text(encoding='utf-8')) if MANIFEST.exists() else {'version':2,'parts':PARTS,'records':[],'policy':{}}
    records=manifest.setdefault('records',[]);by_key={(str(r.get('plant_id')),str(r.get('part'))):r for r in records};plant_by_id={str(p.get('id')):p for p in plants if isinstance(p,dict)}
    changed=False;filled=[];unresolved=[];core=[x for x in idx.get('plants',[]) if x.get('category')=='NCISM-97']
    for ref in core:
        ...
        for part in PARTS:
            ...
            if r.get('image_path') or r.get('verification_status')=='verified':continue
            best=None
            try:
            ...
            accepted=bool(best and ((best.get('source')=='Wikimedia Commons' and best.get('score',0)>=100 and exact_species(best.get('title',''),botanical)) or (best.get('source') in ('iNaturalist','GBIF') and part in ('whole_plant','habit'))))
            if accepted:
                r.update({'image_path':best['url'],'source_url':best['page'],'source_name':best['source'],'license':best['license'],'author':best.get('author',''),'verified_botanical_name':botanical,'verification_status':'verified-external'});changed=True;filled.append({'plant_id':pid,'part':part,'source':best['source'],'title':best['title']})
            else:unresolved.append({'plant_id':pid,'part':part,'reason':'no-high-confidence-image'})
