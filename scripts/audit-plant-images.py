...
USER_AGENT = 'Dravyaguna-97-image-auditor/4.1'
...
def download_image(url, destination):
    ...
    with urllib.request.urlopen(req, timeout=40) as r:
        data = r.read(); content_type = str(r.headers.get_content_type() or '')
    ...
    final = destination.with_suffix('.' + ext); final.write_bytes(data)
...
def materialize_record(r):
    ...
    if src.startswith('images/plants/'):
        path = ROOT / src
        if path.exists() and path.stat().st_size > 2048: return src, 0
...
def main():
    ...
    if int(old.get('version',0) or 0) < 3: old['records'] = []
    manifest = old; records = manifest.setdefault('records', [])
    ...
    for r in records:
        if r.get('source_url') and r.get('verification_status') in ('verified','verified-external','verified-local'): used_sources.add(str(r['source_url']))
        if r.get('image_path') and str(r['image_path']).startswith('images/plants/'): used_local.add(str(r['image_path']))
    for ref in core:
            ...
            if r.get('verification_status') in ('verified','verified-external') and r.get('image_path'):
                try:
                    ...
                    if old_url.startswith('http'):
                        ...
                        if local:
                            r['source_media_url']=old_url; r['image_path']=local; r['verification_status']='verified-local'; used_local.add(local); changed=True
                    ...
                    unresolved.append({'plant_id':pid,'part':part,'reason':'download-failed','detail':str(e)}); r['image_path']=''; r['verification_status']='download-failed'; changed=True
    ...
    for key, slots in seen.items():
        if len(slots)>1:
            ...
            for pid,part in slots[1:]:
                rr=by_key[(str(pid),str(part))]; rr.update({'image_path':'','source_media_url':'','source_url':'','source_name':'','license':'','author':'','verification_status':'duplicate-rejected'}); changed=True
    manifest['version']=4; manifest['parts']=PARTS
