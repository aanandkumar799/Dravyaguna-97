(function(){
  ...
  function init(){apply(preferred());var existing=document.getElementById('themeToggle');if(existing){existing.addEventListener('click',function(){setTimeout(function(){var dark=root.classList.contains('dark-mode');write(dark?'dark':'light');apply(dark?'dark':'light')},0)});return}
    var nav=document.querySelector('nav,.nav,header nav');if(!nav)return;var b=document.createElement('button');b.type='button';b.id='globalThemeToggle';b.className='dg-theme-toggle';b.addEventListener('click',function(){var next=root.classList.contains('dark-mode')?'light':'dark';write(next);apply(next)});nav.appendChild(b);apply(preferred())
  }
