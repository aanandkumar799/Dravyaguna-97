document.addEventListener('DOMContentLoaded', () => {
  ...
  const text = value => Array.isArray(value)
    ? value.map(text).join(' ')
    : (value && typeof value === 'object' ? Object.values(value).map(text).join(' ') : String(value ?? ''));
  ...
  const mergeRecord = (base = {}, patch = {}) => {
    const merged = {
      ...
      dravya_guna: {...(base.dravya_guna || {}), ...(patch.dravya_guna || {})},
      therapeutics: {...(base.therapeutics || {}), ...(patch.therapeutics || {})},
      metadata: {...(base.metadata || {}), ...(patch.metadata || {})}
    };
    if (base.order !== undefined && base.order !== null) merged.order = base.order;
    if (base.category) merged.category = base.category;
    return merged;
  };
  ...
  function score(p, query) {
    ...
    if (!q) return 0;
    const fields = [
      [name(p),1200],[sanskrit(p),1100],[p.transliteration,1050],[botanical(p),1000],
      [p.english_name || p.identity?.english_name,900],[family(p),800],[p.id,750],[p.search_text,200]
    ];
    let total = 0;
  ...
  function filtered(query = '') {
    ...
    return list.map(p => ({p, score: score(p, query)}))
      .filter(x => x.score > 0)
      .sort((a,b) => b.score - a.score || libraryOrder(a.p,b.p))
      .map(x => x.p);
  }
  ...
  function card(p) {
    ...
    const fallback = fallbackSvg(n || 'Plant');
    const imageHtml = img
  ...
  function flashcard(p) {
    const n=name(p), s=sanskrit(p), rasa=text(p.rasa)||'—', guna=text(p.guna)||'—';
    const virya=p.virya||'—', vipaka=p.vipaka||'—', useful=text(p.useful_part)||'—';
  ...
  function render(query='') {
    ...
    if (info) info.textContent = query
      ? `${results.length} result${results.length===1?'':'s'} found for “${query}”`
      : `${results.length} record${results.length===1?'':'s'} in ${categoryFilter==='all'?'the library':categoryFilter}`;
    if (!results.length) {
      plantList.className='plant-grid';
      plantList.innerHTML='<div class="no-results"><div style="font-size:3rem">🔎</div><h3>No plants found</h3><p>Try a Sanskrit name, Roman name, botanical name, family, or NCISM number.</p></div>';
      return;
    }
    ...
    if (mode === 'flash') {
      ...
      plantList.querySelectorAll('.flashcard').forEach(c=>{
        const flip=()=>c.classList.toggle('flipped');
        c.addEventListener('click',e=>{if(!e.target.closest('a'))flip()});
        c.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();flip()}});
      });
    } else {
      plantList.className='plant-grid';
      plantList.innerHTML=visible.map(card).join('') + (!query&&!showAll&&results.length>initialLimit
        ? `<div class="plant-list-actions"><p>Showing ${initialLimit} of ${results.length} records.</p><button type="button" id="show-all-plants">Show all ${results.length}</button></div>` : '');
      plantList.querySelectorAll('img[data-fallback]').forEach(img=>img.addEventListener('error',()=>{
        img.src=img.dataset.fallback;img.removeAttribute('data-fallback');
      },{once:true}));
  ...
  function setupControls() {
    document.querySelectorAll('[data-mode]').forEach(btn=>btn.addEventListener('click',()=>{
      mode=btn.dataset.mode;showAll=true;
      document.querySelectorAll('[data-mode]').forEach(b=>b.classList.toggle('active',b.dataset.mode===mode));
      render(searchInput?.value||'');
    }));
    document.querySelectorAll('[data-category]').forEach(btn=>btn.addEventListener('click',()=>{
      categoryFilter=btn.dataset.category;showAll=false;
      document.querySelectorAll('[data-category]').forEach(b=>b.classList.toggle('active',b.dataset.category===categoryFilter));
      render(searchInput?.value||'');
    }));
    const jump=()=>{
      const n=Number(document.getElementById('ncismJump')?.value);
      if(!n)return;
      const p=masterPlants.find(x=>category(x)==='NCISM-97'&&order(x)===n);
      if(p) location.href=`./plant.html?id=${encodeURIComponent(p.id)}`;
      else alert('NCISM number not found in the current index.');
    };
  ...
  async function load() {
    try {
      let indexPayload;
      try { indexPayload = await fetchJson(urls.index); }
      catch(indexError) { indexPayload = {plants: await fetchJson(urls.fallback)}; }

      const indexed = Array.isArray(indexPayload) ? indexPayload : (indexPayload.plants || []);
      if (!Array.isArray(indexed) || !indexed.length) throw new Error('Invalid plant index.');
      const indexById = new Map(indexed.filter(p=>p?.id).map(p=>[String(p.id),p]));
      masterPlants = [...indexById.values()];

      try {
        const full = await fetchJson(urls.fallback);
        if (Array.isArray(full)) {
          const details = new Map(full.filter(p=>p?.id).map(p=>[String(p.id),p]));
          masterPlants = masterPlants.map(item=>mergeRecord(item,details.get(String(item.id)) || {}));
        }
      } catch(e) { console.warn('Fallback detail database unavailable',e); }

      const batchResults = await Promise.all(urls.batches.map(async url=>{
        try { const batch=await fetchJson(url); return Array.isArray(batch)?batch:[]; }
        catch(e) { console.warn('Batch data unavailable',url,e); return []; }
      }));
      const byId = new Map(masterPlants.map(p=>[String(p.id),p]));
      batchResults.flat().filter(p=>p?.id).forEach(p=>{
        const key=String(p.id);
        byId.set(key,mergeRecord(byId.get(key)||{},p));
      });
      masterPlants = [...byId.values()].filter(p=>p?.id).sort(libraryOrder);
      setupControls();
      plantList.setAttribute('aria-busy','false');
      render(searchInput?.value||'');
    } catch(e) {
      console.error(e);
      plantList.innerHTML=`<div class="plant-error"><h3>Unable to load the plant library</h3><p>${esc(e.message||'Please refresh the page.')}</p></div>`;
      plantList.setAttribute('aria-busy','false');
    }
  }

  searchInput?.addEventListener('input',e=>{
    clearTimeout(searchTimer);showAll=false;
    searchTimer=setTimeout(()=>render(e.target.value),80);
  });
