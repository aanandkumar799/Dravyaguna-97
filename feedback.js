...
function submit(ev){
 ...
 db.collection('reviews').add(payload).then(function(){return db.collection('reviewUsers').doc(user.uid).set({uid:user.uid,displayName:user.displayName||null,email:user.email||null,emailVerified:true,lastSubmittedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true})}).then(function(){document.getElementById('dgFeedbackForm').reset();document.getElementById('dgRating').value='0';document.getElementById('dgCount').textContent='0';document.querySelectorAll('.rating button').forEach(function(x){x.classList.remove('active');x.setAttribute('aria-checked','false')});busy(false);status('Thanks! Your feedback has been submitted.','ok')}).catch(function(e){console.error(e);busy(false);status(e&&e.code==='permission-denied'?'Submission was rejected by the security rules. Please refresh and sign in again.':'Could not submit feedback. Please try again.','err')});
}
