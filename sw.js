const CACHE_NAME = 'dravya-guna-97-v15';
const APP_SHELL = [
...
self.addEventListener('install', event => {
  event.waitUntil(caches.open(CACHE_NAME).then(cache => cache.addAll(APP_SHELL).catch(() => Promise.all(APP_SHELL.map(url => cache.add(url).catch(() => {}))))).then(() => self.skipWaiting()));
});
...
self.addEventListener('fetch', event => {
  ...
  event.respondWith(caches.match(request).then(cached => cached || fetch(request).then(response => {
    if (response.ok && response.type === 'basic') caches.open(CACHE_NAME).then(c => c.put(request, response.clone())).catch(() => {});
    return response;
  })));
});